...
class CrossEvaluation(pydantic.BaseModel):
    ...
    @classmethod
    def from_survey(
        cls, dataset: pandas.DataFrame, args: CrossEvaluationArgs = CrossEvaluationArgs()
    ) -> typing.Self:
            ...
            for human_label, human in group.items():
                for model_label, model in (
                    ...
                    if None not in filter_model_data.to_dict().values():
                        row["value"] = sum(
                            [
                                abs(value - filter_model_data.to_dict()[keys])
            ...
            data=(
                ...
                .pipe(
                    lambda _df: _df.assign(
                        model=pd.Categorical(_df["model"], categories=args.model_order),
    ...
    def plot(self, export_path: str):
        ...
        ax.hlines(
            range(0, len(self.data), len(set(level_1_values))),
            *ax.get_xlim(),
            linewidth=3.0,
            color="white",
        )
